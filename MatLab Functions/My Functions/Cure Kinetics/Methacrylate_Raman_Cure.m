function Methacrylate_Raman_Cure(obj)
...
R = [alpha;T];
if range == 0
    path = fullfile(obj.savefilePath,"Variables",[name,'_full_range.csv']);
    writematrix(R,path);
else
    path = fullfile(obj.savefilePath,"Variables",[name,'_',num2str(range),'s.csv']);
    writematrix(R,path);
end
...
if 1 == strcmp(comp,'mexw64')
    path = fullfile(obj.savefilePath,"Variables",[name,'_',num2str(range),'s_fitparameters','settings']);
